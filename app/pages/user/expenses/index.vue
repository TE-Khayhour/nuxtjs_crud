...
const { mutate: removeMutate } = useConvexMutation(api.expenses.remove)
...
const filteredTotal = computed(() =>
  filteredExpenses.value.reduce((sum: number, e: any) => sum + e.amount, 0)
)
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <div>
        ...
        <p class="text-gray-500 text-sm">
          Showing {{ filteredExpenses.length }} {{ filteredExpenses.length === 1 ? 'item' : 'items' }}
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <select
          ...
          class="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        >
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
      <div v-if="filteredExpenses.length === 0" class="text-gray-400 italic text-center py-12">
        {{ hasFilters ? 'No expenses match your filters.' : 'No expenses yet. Click "+ New Expense" to add one.' }}
      </div>
      <table v-else class="w-full">
        ...
        <tbody>
          ...
          >
            <td class="p-4 text-sm font-medium text-gray-900">{{ exp.title }}</td>
            <td class="p-4">
              <span class="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">{{ exp.category }}</span>
            </td>
            <td class="p-4">
              <span class="text-xs px-2 py-1 rounded-full" :class="exp.paymentMethod === 'Cash' ? 'bg-yellow-50 text-yellow-700' : 'bg-blue-50 text-blue-700'">{{ exp.paymentMethod }}</span>
            </td>
            <td class="p-4 text-sm text-gray-600">{{ exp.date }}</td>
            <td class="p-4 text-sm text-gray-500 max-w-[200px] truncate">{{ exp.note || '-' }}</td>
            <td class="p-4 text-sm font-semibold text-gray-900 text-right">${{ exp.amount.toFixed(2) }}</td>
            <td class="p-4 text-right">
              <div class="flex gap-2 justify-end">
                <NuxtLink
                  :to="`/user/expenses/${exp._id}`"
